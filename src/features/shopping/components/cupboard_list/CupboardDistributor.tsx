...
const CupboardDistributor: FC<Props> = () => {
  ...
  const isCupboardPage = pathname?.includes("cupboard");

  useEffect(() => {
    cupboardSelecteds.length > 0
      ? setHasSelecteds(true)
      : setHasSelecteds(false);
  }, [router, cupboardSelecteds]);
...
export default CupboardDistributor;
